#!/usr/bin/env python3
"""FADED variant (user request 2026-07-12, after the straight-lever economics
test lost -768 to -785pts at 4-8% win): identical fire set, identical entry/
exit mechanics and costs, but take the OPPOSITE side of the lever's call.
Tests whether the same rich-entry/already-priced-in dynamic that killed the
straight bet is instead a fadeable mean-reversion pattern. Same one-shot
discipline: report honestly regardless of outcome, no further variants.
"""
import math
...
for di, day in enumerate(days):
    ...
    for i, (p, sc) in enumerate(zip(bars, scores)):
        ...
        if verdict.side not in ("CE", "PE"):
            abstains += 1
            continue
        faded_side = "PE" if verdict.side == "CE" else "CE"   # <-- FADE the lever
        atm = acc.atm_strike
        entry_val = vert_value(p, atm, faded_side) if atm else None
        ...
        for h in HORIZONS:
            ...
            exits[h] = vert_value(bars[j], atm, faded_side)
        trades.append({"day": day, "i": i, "side": faded_side, "atm": atm,
                        "entry": entry_val, "exits": exits})
        open_until = i + OCCUPY
    if (di + 1) % 50 == 0:
        print(f"  ...{di+1}/{len(days)} days, trades={len(trades)} fires={fires_total} abstain={abstains}", flush=True)

print(f"\nFADED variant (opposite of lever) FIRES={fires_total} abstain={abstains} "
      f"({abstains/max(fires_total,1):.0%}) unpriceable={unpriceable} trades={len(trades)}\n", flush=True)
