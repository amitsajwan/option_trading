...
case "${action}" in
  1)
    "${REPO_ROOT}/ops/gcp/bootstrap_runtime_interactive.sh"
    ;;
  2)
    "${REPO_ROOT}/ops/gcp/start_runtime_interactive.sh"
    ;;
  3)
    "${REPO_ROOT}/ops/gcp/stop_runtime.sh"
    ;;
  4)
    "${REPO_ROOT}/ops/gcp/destroy_infra_preserve_data.sh"
    ;;
  *)
