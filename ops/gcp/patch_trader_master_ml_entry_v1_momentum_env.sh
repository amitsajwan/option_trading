#!/usr/bin/env bash
# E3-S3: ML-only profile (trader_master_ml_entry_v1) + fut_return_5m momentum direction.
# Same voter pool as v1_direction_ml but DIRECTION_ML_MODEL_PATH cleared.
set -euo pipefail
...
echo "Patched $ENV_FILE for trader_master_ml_entry_v1 + momentum direction (no DIRECTION_ML_MODEL_PATH)"
