#!/usr/bin/env bash
# E3-S5: ML-only profile (trader_master_ml_entry_v1) + direction ML bundle.
# No rule strategies in voter pool — only IV_FILTER veto + ML_ENTRY timing/direction.
# Use when det_dir profile shows excessive avoid_veto from TRADER_SKIP / OI_UNWINDING.
set -euo pipefail
...
echo "Patched $ENV_FILE for trader_master_ml_entry_v1 + direction ML (clean ML-only profile)"
