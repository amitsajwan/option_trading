#!/usr/bin/env bash
# E3-S6: ML-only profile (trader_master_ml_entry_v1) + dual direction bundle (CE + PE models).
# Runtime picks whichever side has P(win) > 0.5; neither side → no direction → no trade.
set -euo pipefail
...
DUAL_MODEL="${DIRECTION_ML_MODEL_PATH:-/app/ml_pipeline_2/artifacts/direction_dual/published/direction_dual_model.joblib}"
...
upsert DIRECTION_ML_MODEL_PATH "$DUAL_MODEL"
upsert ML_PURE_RUN_ID ""
upsert ML_PURE_MODEL_GROUP ""
upsert ML_PURE_MODEL_PACKAGE ""
upsert ML_PURE_THRESHOLD_REPORT ""

echo "Patched $ENV_FILE for trader_master_ml_entry_v1 + dual direction ML (CE+PE per-side models)"
