#!/usr/bin/env python3
"""
Full analysis of dir_fix_replay (9e3789a3) vs old run (e8ba040a).
Fields are flat at top-level in strategy_positions_historical.
"""
...
NEW_RUN = "9e3789a3-deb5-4dcd-ba8a-9a646a1033bd"
OLD_RUN = "e8ba040a-a8dd-47d1-9bf8-ceffba85e809"
...
def load_trades(run_id: str) -> list[dict]:
    ...
    for doc in db.strategy_positions_historical.find(
        ...
        rows.append({
            ...
            "stop_pct": stop_pct, "entry_prem": entry_prem, "exit_prem": exit_prem,
...
def summarize(label: str, rows: list[dict]) -> None:
    ...
    for reason, ps in sorted(by_exit.items(), key=lambda x: -len(x[1])):
        ...
        print(f"    {reason:<22} n={len(ps):2d}  wr={wr*100:.0f}%  avg={pct(avg)}")

    print(f"\n  ── Entry strategy ──")
    by_strat: dict[str, list] = defaultdict(list)
    for r in rows:
        by_strat[r["strategy"]].append(r["pnl_pct"])
    for strat, ps in sorted(by_strat.items(), key=lambda x: -len(x[1])):
        wr = sum(1 for p in ps if p > 0) / len(ps)
        print(f"    {strat:<25} n={len(ps):2d}  wr={wr*100:.0f}%  avg={pct(mean(ps))}")
    ...
    for d in ["CE", "PE"]:
        ...
        if dr:
            ...
            print(f"    {d}: n={len(dr):2d}  wr={dw/len(dr)*100:.0f}%  avg={pct(mean(dp))}")

    # Only DET_DIRECTION trades have ML prob
    ml_trades = [r for r in rows if r["ml_prob"] is not None and r["ml_prob"] > 0]
    if ml_trades:
        print(f"\n  ── ML probability bands (DET_DIRECTION trades only, n={len(ml_trades)}) ──")
        bands = [
            ("0.65-0.68", 0.65, 0.68),
            ("0.68-0.72", 0.68, 0.72),
            ("0.72-0.76", 0.72, 0.76),
        ...
        for blabel, lo, hi in bands:
            ...
            if br:
                ...
                print(f"    {blabel:<10} n={len(br):2d}  wr={bw/len(br)*100:.0f}%  avg={pct(mean(bp))}  stops={bstop}")
    ...
    for month in sorted(by_month):
        ...
        print(f"    {month}  n={len(mp):2d}  wr={mw/len(mp)*100:.0f}%  avg={pct(mean(mp))}  total={pct(sum(mp))}")

    print(f"\n  ── Per-trade detail ──")
    print(f"  {'Date':<12} {'Strat':<22} {'D':<3} {'Prob':>5} {'Opt%':>7} {'Cap%':>7} {'MFE':>6} {'MAE':>6} {'Stp':>5} {'Prem':>6} L  Exit")
    print("  " + "-"*105)
    for r in sorted(rows, key=lambda x: x["date"]):
        prob_s = f"{r['ml_prob']:.3f}" if r['ml_prob'] is not None else "  —  "
        print(
            f"  {r['date']:<12} {r['strategy']:<22} {r['direction']:<3} "
            f"{prob_s:>5} "
            f"{pct(r['pnl_pct']):>7} "
...
# ── run ───────────────────────────────────────────────────────────────────────
print("\n" + "█"*65)
print("  DIR-FIX REPLAY ANALYSIS  (multi-signal direction + prob≥0.65)")
print("█"*65)

new_trades = load_trades(NEW_RUN)
old_trades = load_trades(OLD_RUN)

summarize("NEW  (dir_fix, prob≥0.65, 25% stop)", new_trades)
summarize("OLD  (original det_dir, prob≥0.55)", old_trades)

# ── comparison ────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print("  HEAD-TO-HEAD  (all trades)")
print(f"{'='*65}")
for label, trades in [("NEW", new_trades), ("OLD", old_trades)]:
    if not trades:
        continue
    ...
    print(f"    Profit factor  {pf:.2f}")
    print(f"    STOP exits     {len(stops)}/{n} ({len(stops)/n*100:.0f}%)  avg {pct(mean(r['pnl_pct'] for r in stops)) if stops else 'n/a'}")
    print(f"    TARGET exits   {len(tgts)}/{n} ({len(tgts)/n*100:.0f}%)  avg {pct(mean(r['pnl_pct'] for r in tgts)) if tgts else 'n/a'}")
    print(f"    TIME exits     {len(time_s)}/{n} ({len(time_s)/n*100:.0f}%)  avg {pct(mean(r['pnl_pct'] for r in time_s)) if time_s else 'n/a'}")

# DET_DIRECTION-only comparison
print(f"\n{'='*65}")
print("  DET_DIRECTION trades only (ML-gated)")
print(f"{'='*65}")
for label, trades in [("NEW", new_trades), ("OLD", old_trades)]:
    ...
    print(f"  {label} DET_DIRECTION ({len(det)} trades): wr={len(wins)/len(det)*100:.0f}%  avg={pct(mean(pnls))}  total={pct(sum(pnls))}")
