...
DEFAULT_FEATURE_SET_SPECS: List[FeatureSetSpec] = [
    ...
    # Compression-only entry model — no 11:30-anchored velocity.
    # All features computable from the first bar (9:35 IST); model can fire all session.
    # fo_bmm_v1 minus vel_* / ctx_am_* / ctx_gap_* — keeps compression backbone +
    # rolling returns + per-bar OI/PCR + regime context.
    FeatureSetSpec(
        name="fo_compression_v1",
        include_regex=(
            # --- Compression / stored-energy / structure (available from 9:35) ---
            r"^bb_width_20$",
            r"^bb_width_chg_5$",
            r"^bb_width_5m$",
            r"^range_10$",
            ...
            r"^position_in_day_range$",
            r"^compression_score$",
            # --- Structure / trend / vol backbone (rolling, per-bar) ---
            r"^ema_",
            r"^price_vs_vwap$",
            r"^vwap_distance$",
            r"^rsi_14_",
            ...
            r"^vol_spike_ratio$",
            r"^mtf_aligned$",
            # --- OI / PCR per bar (de-correlated confirmers) ---
            r"^pcr$",
            r"^pcr_change_",
            r"^pcr_oi$",
            r"^near_atm_oi_ratio$",
