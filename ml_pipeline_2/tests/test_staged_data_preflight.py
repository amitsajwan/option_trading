...
def test_staged_data_preflight_fails_when_v3_candidate_view_missing_enrichment() -> None:
    """Enrichment coverage check must fire for _v3_candidate views, not just _v2."""
    import json as _json
    tmp_path = Path(tempfile.mkdtemp(prefix="staged-preflight-fail-", dir=Path.cwd()))
    parquet_root = tmp_path / "parquet"
    _write_dataset(parquet_root / "snapshots_ml_flat_v2", _support_frame())

    # View frame deliberately stripped of velocity/enrichment columns
    bare_frame = _view_frame()[
        ["snapshot_id", "instrument", "trade_date", "timestamp",
         "schema_name", "schema_version", "view_name", "build_source", "build_run_id",
         "minutes_since_open", "day_of_week", "ema_9", "price_vs_vwap", "vix_current"]
    ].copy()
    _write_dataset(parquet_root / "stage1_entry_view_v3_candidate", bare_frame)
    _write_dataset(parquet_root / "stage2_direction_view_v3_candidate", bare_frame)
    _write_dataset(parquet_root / "stage3_recipe_view_v3_candidate", bare_frame)
    ...
    payload = {
        ...
        "views": {
            "stage1_view_id": "stage1_entry_view_v3_candidate",
            "stage2_view_id": "stage2_direction_view_v3_candidate",
            "stage3_view_id": "stage3_recipe_view_v3_candidate",
        },
        "labels": {
            "stage1_labeler_id": "entry_best_recipe_v1",
            "stage2_labeler_id": "direction_best_recipe_v1",
    ...
    manifest_path.write_text(_json.dumps(payload, indent=2), encoding="utf-8")

    report = run_staged_data_preflight(manifest_path)

    assert report["status"] == "fail"
    enrichment_errors = [e for e in report["errors"] if "missing planned enrichment coverage" in e]
    assert enrichment_errors, (
        "expected enrichment coverage errors for v3_candidate view with stripped columns, "
