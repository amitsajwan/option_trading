...
def test_staged_manifest_validate_paths_rejects_incompatible_stage1_view_and_support_dataset(tmp_path: Path) -> None:
    parquet_root = tmp_path / "parquet"
    for dataset_name in ("snapshots_ml_flat_v2", "stage1_entry_view", "stage2_direction_view_v2", "stage3_recipe_view_v2"):
        (parquet_root / dataset_name).mkdir(parents=True, exist_ok=True)
    payload = {
        "schema_version": 1,
        "experiment_kind": "staged_dual_recipe_v1",
    ...
    with pytest.raises(
        ...
        match="inputs.support_dataset is incompatible with views.stage1_view_id",
    ):
        resolve_manifest(payload, manifest_path=tmp_path / "staged_incompatible_stage1_support.json", validate_paths=True)
