...
| 2026-05-15 | [MODEL_STATE_20260515.md](MODEL_STATE_20260515.md) | **Two structural tests after honest OOS verdict on C1:** (F1) walk-forward — does recipe survive window shift? (B1) cost-aware label — does it survive `cost_per_trade=0.02`? Plus exit-timing sweep on existing 107 entries. | C1 holdout 16-trade net −55% @ 200 bps. Exit sweep: all 4 hold variants net-negative on holdout. F1 HELD `stage1_cv_gate_failed`. B1 HELD `stage2_signal_check_failed`. | ❌ Both F1 and B1 held. Four independent overfit/data-ceiling signals. Path C-1 (conviction filter) rejected — replicates Apr 26 dead-end. **TRAINING PAUSED.** Next unblocking action: forward shadow data collection (Kite live credentials needed). |
...
## Current Session (2026-05-15) — FINAL: training PAUSED

See [MODEL_STATE_20260515.md](MODEL_STATE_20260515.md) for full detail. Today's arc: started from a "validated +271%" claim; honest OOS decomposition revealed +87% of that gross came from training-window contribution. C1's true holdout (16 trades) is net −55% at 200 bps. Two structural training experiments launched: **F1** (walk-forward, window-shift) HELD at `stage1_cv_gate_failed`; **B1** (cost-in-label shift) HELD at `stage2_signal_check_failed`. Path C-1 (conviction filter) considered and rejected because it replicates the Apr 26 dead-end. **Four independent confirmations that the C1 recipe family has reached the data ceiling on 2020-2024 BankNifty.** Training paused; no further iterations on this dataset. Next unblocking action: forward shadow data collection starting Monday 2026-05-18 09:15 IST — gated on operator sharing Kite live credentials. C1 remains live in `capped_live` paper-only mode; no real money will be deployed.
