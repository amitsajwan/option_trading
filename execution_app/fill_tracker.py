...
class FillTracker:
    ...
    def _handle_fill(self, msg_id: str, fields: dict) -> None:
        try:
            ...
            # ENTRY signals never carry position_id -- it's generated by
            # tracker.open_position() AFTER the TradeSignal object (and its
            # FillEvent) already exist. The guard used to require position_id
            # unconditionally, so it returned early on EVERY entry fill and
            # fill_entry_price/slippage_entry_pct were never written for any
            # live trade (which also meant _get_entry_fill_price always
            # returned None on exit, so fill_pnl_pct was never computed
            # either). Correlate ENTRY fills by signal_id instead --
            # PositionFactory.build() sets PositionContext.signal_id from the
            # same entry signal, and persistence_app indexes strategy_positions
            # on signal_id. Found 2026-07-22.
