...
def _mongo_latest() -> tuple[bool, dict[str, Any], Optional[str]]:
    ...
    try:
        ...
        return False, {"db": db_name, "collection": coll_name}, str(exc)


def evaluate(*, max_age_seconds: float) -> tuple[dict[str, Any], int]:
    process_matches = find_matching_python_processes(["persistence_app.main_snapshot_consumer"])
...
def run_cli(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Health check for persistence_app")
