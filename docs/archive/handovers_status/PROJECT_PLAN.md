...
| **Known gaps** | (1) `exit_trigger` field is computed by the tracker but not persisted to mongo — low-severity diagnostic loss. (2) No broker integration — no real-money orders possible regardless of model decision (Phase 4 work). |
