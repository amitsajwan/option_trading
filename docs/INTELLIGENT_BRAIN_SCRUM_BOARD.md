...
**UPDATE 2026-06-07 (REAL per-fill calibration — verdict flips POSITIVE, reconciles with live):** The −1.44%/−4.68% above used a WRONG assumed premium (₹180). Walking the **actual held-strike option path** (real ce/pe ltp from the chain; avg ATM premium **₹1044**) flips it: **net = +0.09%/trade (time-stop) → +0.49%/trade (giveback-fix)** over the 11 decided trades. Per-day: 05-26 +11.2%, 05-27 −9.8% (the drag; hard-stop cut it from −14%), 06-02 +4.8%, 06-03 +4.0%, 06-04 −4.8%; **06-05 = ZERO trades (brain abstained, avoided the −6.1% disaster day).** Winner MFE: max **32.9%**, 1 trade >15%, 2 >5% (confirms live "15%+ runners"). **+0.49%/trade × ₹1044 × 30 lot ≈ ₹1,690 ≈ ~$20 — matches the operator's live profit.** So the move/direction/exit path is **net positive on real prices** (modeled; n=11, 8 quiet days, exit fills use ltp w/ sparse intrabar high/low). The earlier "MARGINAL/negative" was a premium-assumption artifact. **Lesson: never trust proxy P&L magnitudes — calibrate on real fills.**
