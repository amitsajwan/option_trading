...
class ModelSwitchResolverTests(unittest.TestCase):
    def _prepare_repo(self, root: Path, *, run_id: str, decision: str = "PROMOTE", create_artifacts: bool = True) -> Path:
        reports_dir = (
            root
            / "ml_pipeline"
            / "artifacts"
        ...
        model_path = root / "ml_pipeline" / "artifacts" / "models" / "by_features" / "banknifty_futures" / "h15_tp_auto" / "model" / "model.joblib"
        threshold_path = root / "ml_pipeline" / "artifacts" / "models" / "by_features" / "banknifty_futures" / "h15_tp_auto" / "config" / "profiles" / "x" / "threshold_report.json"
        if create_artifacts:
    ...
    def test_resolve_ml_pure_artifacts_success(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            self._prepare_repo(root, run_id="20260308_164057", decision="PROMOTE", create_artifacts=True)
    ...
    def test_validate_switch_strict_blocks_non_promote(self) -> None:
        payload = {
            "promotion_decision": {"decision": "HOLD"},
            "published_paths": {"model_package": "a", "threshold_report": "b"},
        }
        ok, reason = validate_switch_strict(payload)
        self.assertFalse(ok)
        self.assertIn("decision=HOLD", reason)
    ...
    def test_validate_switch_strict_checks_missing_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            run_path = self._prepare_repo(root, run_id="20260308_164057", decision="PROMOTE", create_artifacts=False)
