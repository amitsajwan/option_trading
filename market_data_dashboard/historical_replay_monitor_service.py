...
class HistoricalReplayMonitorService(LiveStrategyMonitorService):
    ...
    def get_replay_status(self, *, date: Optional[str] = None, instrument: Optional[str] = None) -> dict[str, Any]:
        ...
        completed = str(replay.get("status") or "").strip().lower() in {"complete", "completed", "no_snapshots"}
        return {
            "mode": "historical",
            "dataset": "historical",
            ...
            "data_ready": bool(replay.get("data_ready")),
            "completed": completed,
            "status": replay.get("status") or ("ready" if replay.get("data_ready") else "idle"),
