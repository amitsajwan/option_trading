...
class StrategyEvaluationService:
    ...
    def _strategy_profile_ids_for_runs(self, run_ids: list[str], dataset: str) -> dict[str, str]:
        ...
        ids = [str(rid or "").strip() for rid in run_ids if str(rid or "").strip()]
        if not ids:
            return {}
        mode = str(dataset or "historical").strip().lower()
        names = self._collection_names(mode)
                ...
                pipeline = [
                    {
                        "$match": {
                            "run_id": {"$in": remaining},
                            "strategy_profile_id": {"$nin": [None, ""]},
                        }
                    },
                    {"$group": {"_id": "$run_id", "strategy_profile_id": {"$first": "$strategy_profile_id"}}},
                ]
                for row in self._db()[names[coll_name]].aggregate(pipeline, allowDiskUse=True):
                    rid = str(row.get("_id") or "").strip()
                    pid = str(row.get("strategy_profile_id") or "").strip()
                    if rid and pid:
                        out[rid] = pid
            except Exception:
                pass
    ...
    @staticmethod
    def _run_list_sort_key(row: dict[str, Any]) -> str:
        """Sort runs by execution time (ended → submitted → updated), not replay window."""
        for field in ("ended_at", "submitted_at", "updated_at"):
            text = str(row.get(field) or "").strip()
            if text:
                return text
        if row.get("discovered"):
            end_day = str(row.get("date_to") or "").strip()
            if end_day:
                return f"{end_day}T23:59:59+05:30"
        return str(row.get("run_id") or "")

    def enrich_run_metadata(self, doc: dict[str, Any], *, dataset: str = "historical") -> dict[str, Any]:
        """Attach strategy_profile_id from trades when the run registry row lacks it."""
        if not isinstance(doc, dict):
            return doc
        if str(doc.get("strategy_profile_id") or "").strip():
            return doc
        rid = str(doc.get("run_id") or "").strip()
        if not rid:
            return doc
        mode = str(doc.get("dataset") or dataset).strip().lower()
        profiles = self._strategy_profile_ids_for_runs([rid], mode)
        pid = profiles.get(rid)
        if not pid:
            return doc
        enriched = dict(doc)
        enriched["strategy_profile_id"] = pid
        return enriched
    ...
    def list_runs(
        ...
        *,
        ...
        if rows and mode == "historical":
            run_ids = [str(row.get("run_id") or "").strip() for row in rows if str(row.get("run_id") or "").strip()]
            profiles = self._strategy_profile_ids_for_runs(run_ids, mode)
            for row in rows:
                rid = str(row.get("run_id") or "").strip()
                if rid and profiles.get(rid) and not str(row.get("strategy_profile_id") or "").strip():
                    row["strategy_profile_id"] = profiles[rid]

        rows.sort(key=self._run_list_sort_key, reverse=True)
