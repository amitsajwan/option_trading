...
(function () {
  ...
  function fmtPctPoint(value) {
    ...
    return C.fmtSigned(num * 100, 1, 'pp');
  }
  ...
  function buildByStrategyRows(currentRows, baselineRows) {
    ...
    currentRows.forEach(function (row) {
      map[row.label] = { name: row.label, curTr: Number(row.trades || 0), curR: Number(row.value || 0), baseTr: 0, baseR: 0 };
    });
    baselineRows.forEach(function (row) {
      var slot = map[row.label] || { name: row.label, curTr: 0, curR: 0, baseTr: 0, baseR: 0 };
  ...
  function buildVerdict(current, baseline) {
    ...
    var gates = [
      { label: 'Net return delta >= +0.25%', value: returnDelta,   threshold: 0.0025, format: function (v) { return C.fmtSigned(v * 100, 2, '%'); } },
      { label: 'Max drawdown delta >= 0.00%', value: drawdownDelta, threshold: 0.0,   format: function (v) { return C.fmtSigned(v * 100, 2, '%'); } },
      ...
      return { label: gate.label, value: Number.isFinite(gate.value) ? gate.format(gate.value) : '--', pass: pass };
    });
  ...
  function buildPageData(current, baseline) {
    ...
    return {
      ...
      kpis: [
        ...
        { label: 'DELTA RETURN',    value: Number.isFinite(returnDelta) ? C.fmtSigned(returnDelta * 100, 2, '%') : '--', cls: Number.isFinite(returnDelta) && returnDelta >= 0 ? 'pos' : 'neg', sub: 'Current - baseline' },
        ...
        { label: 'VERDICT',         value: verdict.label, cls: verdict.cls, sub: verdict.gates.filter(function (g) { return g.pass; }).length + '/' + verdict.gates.length + ' gates pass' },
      ],
    };
  }
  ...
  function getFallbackData() {
    ...
    var current = buildScenario('Current live session', 'live_2026-04-18', {
      kpis: { tradeCount: 7, netReturn: '+1.62%', netReturnCls: 'pos', winRate: 0.71, maxDrawdown: '-0.22%', profitFactor: 3.4, sharpe: 2.81, avgHoldBars: 16 },
      overall: { trade_count: 7, net_return_pct: 0.0162, win_rate: 0.71, max_drawdown_pct: -0.0022 },
      byStrategy: [
    ...
    var baseline = buildScenario('Latest historical run', 'run_hist_20260411', {
      kpis: { tradeCount: 11, netReturn: '+0.98%', netReturnCls: 'pos', winRate: 0.63, maxDrawdown: '-0.41%', profitFactor: 2.1, sharpe: 1.94, avgHoldBars: 24 },
      overall: { trade_count: 11, net_return_pct: 0.0098, win_rate: 0.63, max_drawdown_pct: -0.0041 },
      byStrategy: [
      ...
      '<div class="g-main-side">' +
        ...
        '<div class="panel">' +
          ...
          '<div class="panel-body">' +
            '<div style="font-size:15px; font-weight:600; color:var(--ink); letter-spacing:-0.01em">' + C.esc(verdict.summary) + '</div>' +
            '<div class="hr"></div>' +
            '<div style="display:grid;gap:10px">' + verdict.gates.map(function (gate) {
              return '<div class="rowx" style="font-size:12px"><span>' + C.esc(gate.label) + '</span><span class="chip ' + (gate.pass ? 'pos' : 'warn') + '"><span class="dot"></span>' + C.esc(gate.value) + '</span></div>';
            }).join('') + '</div>' +
            '<div class="hr"></div>' +
          ...
          '<div class="panel-body flush">' +
            '<table class="tbl">' +
              ...
              '<tbody>' + data.compareRows.map(function (row) {
                return '<tr>' +
                  '<td>' + C.esc(row.name) + '</td>' +
                  '<td class="r" style="font-weight:600">' + C.esc(row.current) + '</td>' +
                  '<td class="r muted">' + C.esc(row.baseline) + '</td>' +
                  '<td class="r">' + (row.dir === 'pos' ? '<span class="chip pos">up</span>' : (row.dir === 'neg' ? '<span class="chip neg">down</span>' : '')) + '</td>' +
                '</tr>';
              }).join('') + '</tbody>' +
            '</table>' +
          '</div>' +
            ...
            '<table class="tbl">' +
              ...
              '<tbody>' + data.byStrategyRows.map(function (row) {
                return '<tr>' +
                  ...
                  '<td class="r ' + (row.curR >= 0 ? 'pos' : 'neg') + '">' + C.fmtSigned(row.curR, 2, '%') + '</td>' +
                  '<td class="r muted">' + row.baseTr + '</td>' +
                  '<td class="r muted">' + C.fmtSigned(row.baseR, 2, '%') + '</td>' +
                  '<td class="r ' + (row.delta >= 0 ? 'pos' : 'neg') + '">' + C.fmtSigned(row.delta, 2, '%') + '</td>' +
                '</tr>';
              }).join('') + '</tbody>' +
            '</table>' +
          '</div>' +
