...
def test_classify_run_id_pattern_match():
    ...
    from market_data_dashboard.services.historical_replay_monitor_service import _classify_run_id
...
def test_state_includes_runtime_config_and_available_models(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "market_data_dashboard.state.strategy_current_state.DEFAULT_PUBLISHED_MODELS_ROOT",
        tmp_path / "no-models",
    )
